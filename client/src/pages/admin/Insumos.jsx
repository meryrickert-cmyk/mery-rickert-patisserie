    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 10 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          {snapMsg && <span style={{ fontSize: 23, color: '#2d7a3a', fontWeight: 500 }}>{snapMsg}</span>}
      ...
      {pkgItems.length > 0 && (
        <div style={{ marginBottom: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
            <span style={{ fontSize: 23, color: 'var(--texto-suave)', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600 }}>Packaging (transversal a todas las recetas)</span>
          ...
          <thead>
            <tr style={{ borderBottom: '1px solid var(--crema-oscuro)', background: 'var(--crema)' }}>
              {['Insumo', 'Unidad', 'Costo actual', '', 'Actualizado'].map(h => (
                <th key={h} style={{ padding: '11px 16px', textAlign: 'left', fontSize: 16, color: 'var(--texto-suave)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.07em', fontFamily: 'var(--sans)' }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
      ...
      <p style={{ fontSize: 23, color: 'var(--texto-suave)', marginTop: 10 }}>
      ...
      {modal && (
        <Modal title={modal === 'nuevo' ? 'Nuevo insumo' : `Editar: ${modal.nombre}`} onClose={() => setModal(null)}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <p style={{ fontSize: 23, color: 'var(--texto-suave)', margin: 0 }}>Al guardar se registra el costo en el historial del mes actual.</p>
          </div>
        ...
        return (
          <div key={cat} style={{ marginBottom: 40 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 0 }}>
              <span style={{ fontSize: 23, color: 'var(--texto-suave)', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600 }}>
                {CAT_LABEL[cat] || cat}
              </span>
              <span style={{ fontSize: 23, color: 'var(--texto-suave)', background: 'var(--crema)', borderRadius: 20, padding: '1px 8px' }}>{recsDecat.length}</span>
              <div style={{ flex: 1, height: 1, background: 'var(--crema-oscuro)' }} />
            </div>
                  ...
                  <div key={rec.id} style={{ borderBottom: (!isLast || isOpen) ? '1px solid var(--crema-oscuro)' : 'none' }}>
                    ...
                    {isOpen && (
                      <div style={{ padding: '0 16px 14px', borderTop: '1px solid var(--crema-oscuro)', background: '#faf9f7' }}>
                        <p style={{ fontSize: 23, color: 'var(--texto-suave)', margin: '10px 0 8px' }}>
                                    ...
                                    {isEditing ? (
                                      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                                        <input type="number" value={editingQty.valor}
                                          ...
                                          autoFocus
                                          style={{ width: 70, padding: '4px 8px', borderRadius: 6, border: '1.5px solid var(--bordeaux)', fontSize: 23, textAlign: 'center' }} />
                                        <span style={{ fontSize: 23, color: 'var(--texto-suave)' }}>{unidadLabel}</span>
                                      </div>
                                    ) : (
                                      <span onClick={() => setEditingQty({ ingId: ing.id, valor: ing.cantidad })}
                                        style={{ cursor: 'pointer', color: 'var(--texto-suave)', fontSize: 23, borderBottom: '1px dashed #ccc', paddingBottom: 1 }}
        ...
        <Modal title={modal === 'nuevo' ? 'Nueva receta' : `Editar: ${modal.nombre}`} onClose={() => setModal(null)} wide>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div>
                ...
                return (
                  <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 8 }}>
                    ...
                    <div style={{ flex: 1, position: 'relative' }}>
                      ...
                      {sel && <span style={{ position: 'absolute', right: 10, top: '50%', transform: 'translateY(-50%)', fontSize: 23, color: 'var(--texto-suave)' }}>{sel.unidad === 'kg' || sel.unidad === 'L' ? 'g' : 'u'}</span>}
                    </div>
              ...
              {form.ingredientes.length === 0 && <p style={{ fontSize: 23, color: 'var(--texto-suave)' }}>Hacé clic en "+ Ingrediente" para agregar.</p>}
            </div>
          </div>
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 20 }}>
            <button onClick={() => setModal(null)} style={btnSecundario}>Cancelar</button>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 23, color: 'var(--texto-suave)' }}>Período:</span>
                ...
                {filtrados.map((d, i) => {
                  ...
                  return (
                    <tr key={d.id} style={{ borderBottom: i === filtrados.length - 1 ? 'none' : '1px solid #f5f0eb' }}>
                      <td style={{ padding: '11px 14px', fontSize: 23, fontFamily: 'var(--serif)', color: 'var(--texto)', minWidth: 160 }}>
                        {d.nombre}
                        {d.producto_nombre && <span style={{ fontSize: 21, color: 'var(--texto-suave)', display: 'block', fontFamily: 'var(--serif)' }}>→ {d.producto_nombre}</span>}
                      </td>
                      ...
                      <td style={{ ...tdR }}>
                        {d.margen_pct !== null
                          ? <span style={{ background: c + '18', color: c, padding: '3px 10px', borderRadius: 50, fontSize: 23, fontWeight: 700 }}>{d.margen_pct.toFixed(1)}%</span>
                          : '—'}
                      </td>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={onClose}>
      <div style={{ background: '#fff', borderRadius: 18, padding: '28px 28px 24px', width: '100%', maxWidth: wide ? 720 : 460, maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          ...
          <button onClick={onClose} style={{ border: 'none', background: 'none', fontSize: 26, cursor: 'pointer', color: 'var(--texto-suave)', lineHeight: 1 }}>×</button>
        </div>
        {children}
      </div>
    </div>
...
function Kpi({ label, value, color }) {
  return (
    <div style={{ background: 'var(--crema)', borderRadius: 10, padding: '13px 16px' }}>
      ...
      <p style={{ fontSize: 26, fontWeight: 700, color: color || 'var(--texto)', margin: 0 }}>{value}</p>
    </div>
  );
}
